...
describe('buildClaudeCommand', () => {
  ...
  it('should build basic command without session ID (auto mode)', () => {
    ...
    expect(command).toContain("cd /test/dir && claude --output-format stream-json --model claude-opus-5-5 -p --verbose");
  ...
  it('should build command with session ID (auto mode)', () => {
    ...
    expect(command).toContain("cd /test/dir && claude --resume session-123 --output-format stream-json --model claude-opus-5-5 -p --verbose");
    expect(command).toMatch(/ -- 'hello world'$/);
    expect(command).toContain('--dangerously-skip-permissions');
  });
  ...
  it('should handle complex prompts', () => {
    ...
    expect(command).toContain("claude --resume abc-123 --output-format stream-json --model claude-opus-5-5 -p --verbose");
