...
export class MCPPermissionServer {
  ...
  private setupRoutes(): void {
    ...
    // Schedule a one-shot wakeup: re-invoke cc in this thread after a delay.
    this.app.post('/tool/schedule_wakeup', (req, res) => {
      console.log('HTTP schedule_wakeup request:', req.body);

      const discordContext = this.extractDiscordContext(req) || req.body.discord_context;
      if (!this.db || !discordContext) {
        res.json({ error: 'Schedule wakeup not available (no DB or Discord context).' });
        return;
      }
      ...
      const session = this.db.getThreadSession(threadId);
      if (!session) {
        res.json({ error: 'No session found for this thread; cannot schedule a wakeup.' });
        return;
      }

      const { prompt, delay_seconds, reason } = req.body ?? {};
      if (!prompt || typeof prompt !== 'string') {
        res.json({ error: 'A non-empty "prompt" string is required.' });
        return;
      }

      const rawDelay = typeof delay_seconds === 'number' ? delay_seconds : parseFloat(delay_seconds);
      if (!Number.isFinite(rawDelay) || rawDelay < 60) {
        res.json({ error: '"delay_seconds" must be a number >= 60.' });
        return;
      }
      const delaySeconds = Math.min(Math.floor(rawDelay), 3600);

      const id = `wakeup-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
      const now = Date.now();
      const wakeAt = now + delaySeconds * 1000;
      ...
      try {
        this.db.createScheduledTask({
          ...
          userId: discordContext.userId,
          prompt,
          label: typeof reason === 'string' && reason ? reason : undefined,
        ...
        res.json({
          ...
          id,
          wakeAt,
          wakeInSeconds: delaySeconds,
          note:
            `Wakeup scheduled in ~${formatInterval(delaySeconds)}. ` +
            `End your turn now — the bot will re-invoke you in this thread when the delay elapses.`,
        });
      } catch (error) {
        console.error('HTTP schedule_wakeup error:', error);
        res.json({ error: error instanceof Error ? error.message : String(error) });
      }
    });
