...
export interface ThreadSession {
  ...
  goal?: string;
  // Bot to hand off to when agent completes (e.g. "hermes"). Set via /handoff.
  handoffBot?: string;
}
...
export class DatabaseManager {
  ...
  setThreadHandoffBot(threadId: string, botName: string | null): void {
    this.db
      .prepare(`UPDATE thread_sessions SET handoff_bot = ? WHERE thread_id = ?`)
      .run(botName, threadId);
  }

  getThreadHandoffBot(threadId: string): string | null {
    const row = this.db
      .prepare(`SELECT handoff_bot FROM thread_sessions WHERE thread_id = ?`)
      .get(threadId) as { handoff_bot: string | null } | undefined;
    return row?.handoff_bot ?? null;
  }
